...
void debug_in(int type,void * handle,int endpoint)
{
	//if(type==0X00)control_transfer(handle,endpoint);
	//if(type==0X01)isochromous_transfer(handle,endpoint);
	//if(type==0X02)bulk_transfer(handle,endpoint);
	if(type==0X03)interrupt_transfer(handle,endpoint);
}
void debug_out(int type,void * handle,int endpoint)
{}